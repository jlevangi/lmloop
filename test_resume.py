...
class AttachTests(unittest.TestCase):
    ...
    def test_the_floor_never_exceeds_the_ceiling(self):
        """A saved ceiling well above the work done is the case that actually
        distinguishes clamping the floor from not: `done + extra` (8) sits
        below `ceiling` (23), so a fixture where the two already coincide
        would pass whether the floor is clamped down or left at the ceiling."""
        run = self.make_run(state={"hard_turn_ceiling": 20}, iterations_done=5)
        self.attach(run, extra=3)
